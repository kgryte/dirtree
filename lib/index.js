...
(function() {
	...
	* @param {RegExp} [filter] - regular expression used for file inclusion
	...
	*
	* @param {RegExp} [include] - regular expression for leaf inclusion
	* @param {RegExp} [exclude] - regular expression for leaf exclusion  
	...
	*/
	Tree.prototype.search = function( include, exclude ) {
		...
		if ( !arguments.length ) {
			throw new Error( 'search()::insufficient input arguments. Must provide a regular expression filter.' );
		}
	...
	* @returns {Tree} Tree instance
	*/
	Tree.prototype.read = function( options, clbk ) {
		var leaves = this._leaves,
			tree = this._flattened,
			numLeaves = leaves.length,
			matches = [],
